...
function TCOMBO(str) {
  const [name, press, inner, outer, both, left, right] = str.split(',').map(s => s.trim())
  combos.push([
    name, press, inner, outer, both, left, right
  ])
}

const COMBO_TIMEOUT = 200
const STICKY_TIMEOUT = 500
const COMBO_LAYERS = 0
...
const BLM = 10 // BOTTOM
const BLI = 11 // BOTTOM
const LOT = 16 // O-THUMB
const LIT = 17 // I-THUMB
...
const BRM = 13 // BOTTOM
const BRI = 12 // BOTTOM
const ROT = 19 // O-THUMB
const RIT = 18 // I-THUMB
...
console.log(combos.map(template).join('\n\n'))
